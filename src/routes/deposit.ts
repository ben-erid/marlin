...
function requireSession(req: any, res: any, next: any) {
  ...
  if (!session) return res.status(401).json({ error: 'Invalid or expired session — call POST /api/auth/connect first' });
  req.session = session;
  next();
}
...
router.get('/:trackCode', requireSession, async (req, res) => {
  ...
  try {
    const session = getSession(sid)!;
    const page = await session.context.newPage();
    const result = await getOrderStatus(page, trackCode);
    await page.close();
    return res.json(result);
  } catch (err: any) {
